...
function StatCard({ label, value, sub, color }: { label: string; value: number | string; sub?: string; color: string }) {
  return (
    <div className="rounded-xl p-4" style={{ background: '#0a0b10', border: `1px solid ${color}20` }}>
    ...
    <div className="flex-1 relative z-10">
      ...
      <div className="px-6 py-5 space-y-5 max-w-3xl">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <StatCard label="Total Leads"   value={data.totalLeads}   sub="all time"        color="#67e8f9" />
        ...
        {/* Lead pipeline */}
        <div className="rounded-xl p-5" style={{ background: '#0a0b10', border: '1px solid rgba(6,182,212,0.1)' }}>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <TrendingUp size={14} className="text-cyan-400" />
              <p className="text-xs font-semibold text-slate-300 uppercase tracking-widest">Lead Pipeline</p>
            </div>
            <Link href="/freelancer/leads" className="text-[11px] text-cyan-500/60 hover:text-cyan-400 transition-colors flex items-center gap-1">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="rounded-xl p-4" style={{ background: '#0a0b10', border: '1px solid rgba(100,116,139,0.1)' }}>
            <div className="flex items-center justify-between mb-3">
              ...
              <Link href="/freelancer/leads" className="text-[10px] text-cyan-500/60 hover:text-cyan-400 transition-colors">All →</Link>
            </div>
            <div className="space-y-2">
              {data.recentLeads.length === 0 ? (
                <p className="text-[11px] text-slate-600 py-4 text-center">No leads yet</p>
              ) : data.recentLeads.map((lead: any) => (
                <div key={lead.id} className="flex items-center gap-2.5 py-1.5 border-b border-slate-800/40 last:border-0">
          ...
          <div className="rounded-xl p-4" style={{ background: '#0a0b10', border: '1px solid rgba(100,116,139,0.1)' }}>
            <div className="flex items-center justify-between mb-3">
              ...
              <Link href="/freelancer/applied" className="text-[10px] text-cyan-500/60 hover:text-cyan-400 transition-colors">All →</Link>
            </div>
            <div className="space-y-2">
              {data.recentApplied.length === 0 ? (
                ...
                <div key={job.id} className="flex items-center gap-2.5 py-1.5 border-b border-slate-800/40 last:border-0">
                  <div className="flex-1 min-w-0">
                  ...
                  <a href={job.url} target="_blank" rel="noreferrer" className="text-slate-600 hover:text-cyan-400 transition-colors shrink-0">
